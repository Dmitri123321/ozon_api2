...
def make_index(app):
    ...
    collections_list = [app.collection_products, app.collection_prices, app.collection_stocks,
                        app.collection_analytics, app.collection_transaction]
    for ind, index in enumerate(indexes_list):
        indexes = collections_list[ind].index_information()
        if not check_index(indexes, index):
            collections_list[ind].create_index(index, unique=True)
...
def send_dayly_analitics(app, items):
    pass
